...
def print_state_info(state):
    ...
    if state.get('generated_sql'):
        print(f"\nGenerated SQL:")
        print(f"Purpose: {state.get('sql_purpose', 'N/A')}")
        print(f"Query: {state['generated_sql'][:200]}..." if len(state.get('generated_sql', '')) > 200 else f"Query: {state.get('generated_sql', 'N/A')}")
...
def print_tool_results(messages):
    ...
    for i, tool_msg in enumerate(tool_messages, 1):
        try:
            ...
            if truncated:
                print(f"  (Results truncated at {row_count} rows)")
...
def main():
    print("="*80)
    print("PHARMA MODEL RESULTS INTERPRETER (SQL-BASED)")
    print("="*80)
    print("\nType 'quit' to exit")
    ...
    try:
        while True:
            ...
            if user_input.lower() == 'help':
                ...
                print("  - Find all models using XGBoost algorithm")
            ...
            conversation_state["generated_sql"] = None
